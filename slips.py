...
# 20200710 - port of javascript to psychopy
...
Dir = Enum("Dir", "None Left Right")
# Box has a 'S' stimulus outside, and 'O' outcome on the inside */
class Box:
    def __init__(self, S: Fruit, O: Fruit):
        self.S = S
        self.O = O

NUMKEYS : List[int] = [49, 50, 51, 52, 53, 54] # keycodes for 1-6 in order
...
def allBoxes(frts:List[Dict[string, Fruit]], soa_boxes: List[List[int]]) -> List[Box]:
    fruit_names : List[string] = shuffle(frts.keys())
    nboxes = soa_boxes.length;
    if(nboxes != fruit_names.length/2): Exception("nboxes != nfruits/2")
    sides = shuffle(repeat([Dir.Left, Dir.Right], nboxes/2))
    boxes = [ mkBox(frts[fruit_names[i]], frts[fruit_names[nboxes+i]], sides[i], devalidxs) for (devalidxs,i) in enumerate(soa_boxes)]
    return boxes


def random_IDidx(n: int) -> List[int]:
    # repeat 0-5 twice
    idxs: List[int] = [];
    for j in range(SETTINGS['ID_reps']):
        for i in range(n):
            idxs.push(i);
    ...
    for i in range(SETTINGS['ID_blocks']):
        idxlist.push(shuffle(idxs))
    
    # flatten and return
    return functools.reduce(operator.iconcat, indexlist, [])
